import numpy as np
import torch
from torchvision.transforms.v2.functional import normalize, to_dtype, resize
...
def nvav_collator(batch):
    ...
    for item in batch:
        ...
        vel_list.append(torch.tensor(vel))   # (N, 3)
        acc_list.append(torch.tensor(acc))   # (N, 3)
        goal_list.append(torch.tensor(goal)) # (N,)
        for cam in range(D):
            img_list[cam].append(item["image_features"][cam])
        gt_list.append(torch.tensor(item["gt_proposals"]))
        ext_list.append(torch.tensor(item["extrinsics"], dtype=torch.float32))
        int_list.append(torch.tensor(item["intrinsics"]))
        vdims_list.append(torch.tensor(item["vehicle_dims"]))
...
class NVAVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for camera_name in self.camera_names:
            camera_readers.append(self.ds.get_clip_feature(clip_id, camera_name, maybe_stream=True))


        camera_readers = tuple(camera_readers)
        ego_reader = self.ds.get_clip_feature(clip_id, "egomotion", maybe_stream=True)
        ...
        # frame_idx = np.arange(0, n_frames, dtype=int)
        ...
        for reader in camera_readers:
            ...
            frame = torch.tensor(frame.transpose(0, 3, 1, 2)) # (N, C, H, W)
            
            downsampled_size = tuple(int(self.downsample * s) for s in frame.shape[-2:])
            frame = resize(frame, size=downsampled_size)
            frame = to_dtype(frame, dtype=torch.float32, scale=True) # scale=True normalizes images from [0, 255] to [0, 1]
            frame = normalize(frame, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # normalize with image net mean and std

            frames.append(frame) 


        # camera intrinsics, extrinsics, and vehicle dimensions
        reader = self.ds.get_clip_feature(clip_id, "sensor_extrinsics", maybe_stream=True)
        extrinsics = reader.loc[self.camera_names].to_numpy() # each row qx, qy, qz, qw, x, y, z

        reader = self.ds.get_clip_feature(clip_id, "camera_intrinsics", maybe_stream=True)
        intrinsics = reader.loc[self.camera_names, ["width", "height", "cx", "cy", "fw_poly_0","fw_poly_1",  "fw_poly_2",  "fw_poly_3",  "fw_poly_4"]].to_numpy()
        
        reader = self.ds.get_clip_feature(clip_id, "vehicle_dimensions", maybe_stream=True)
...
def plot_bev_trajectory(clip_id, ds, num_points=100):
    reader = ds.get_clip_feature(clip_id, "egomotion", maybe_stream=True)
